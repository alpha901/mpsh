	...
	while(mot!=NULL){
		...
		//mots suivants
			...
			if((strcmp(mot,"||")==0)||(strcmp(mot,"&&")==0)||(strcmp(mot,"|")==0)){
				...
				char * commande2 = strtok(tmp2,mot);
				commande2 = strtok(NULL,mot);
				if ((strcmp(mot,"||")==0)||(strcmp(mot,"&&")==0))
					*contientConnecteur = 1;
				else if (strcmp(mot,"|")==0)
					*contientPipe = 1;
				short traitementDeLaSuiteDeCommande = commandeCorrecte(commande2+1,motsDeLaCommande+nbMots,indiceRedirection,affectation,contientConnecteur,contientPipe);
				if(!traitementDeLaSuiteDeCommande)
						return 0;
				nbMots += traitementDeLaSuiteDeCommande;
			}else{
				// s'il n'existe pas de connecteur ou pipe
	...
	if(nbMots!=-1){  //si la commande est vide on ne la traite pas
		...
		if(elementsDeLaCommande==NULL){
			...
			*traitementReussi = 0;
			...
			else{ //On traite des commandes internes
